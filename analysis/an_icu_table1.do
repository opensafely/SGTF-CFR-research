...
* Open a log file
cap log close
log using ./logs/an_icu_table1, replace t
...
noi di "SUBSETTING ON ICU ADMISSIONS"
keep if icu_pop == 1
...
tabulatevariable, variable(icu_pop) min(1) max(1) 
file write tablecontent _n
...
*EPI WEEK
tabulatevariable, variable(start_weekB) min(1) max(8) 
file write tablecontent _n 
...
graph export ./output/time_death_icu.svg, as(svg) replace
...
insheet using ./output/table1_icu.txt, clear

export excel using ./output/table1_icu.xlsx, replace
